...
namespace Common.Payment
{
    ...
    /// 注意：请求的价格(单位：元)
    /// </summary>
    public class cbbpay : PaymentBase, IPayment
    {
        #region 充值[Recharge]
        /// <summary>
        /// 常量：充值请求待签模板（请求前 签名）
        ...
        protected override string RechargeRequestSignTemplate
        {
            get
            {
                return "amount={amount}&ip={ip}&mch_id={mch_id}&notify_url={notify_url}&out_trade_no={out_trade_no}&success_url={success_url}&type={type}&key={key}";
            }
        }
        ...
        protected override string RechargeRequestDataTemplate
        {
            get
            {
                return "amount={amount}&ip={ip}&mch_id={mch_id}&notify_url={notify_url}&out_trade_no={out_trade_no}&sign={sign}&success_url={success_url}&type={type}";
            }
        }
        ...
        protected override string RechargeNotifySignTemplate
        {
            get
            {
                return "amount={amount}&charge_type={charge_type}&mch_id={mch_id}&order_no={order_no}&out_trade_no={out_trade_no}&pay_money={pay_money}&status={status}&key={key}";
            }
        }
        ...
        public RechargeResult Recharge(string shop, string bank, string order, DateTime time, decimal amount, string notify, string redirect, string referer, string ip, string key, string info, string url)
        {
            ...
            string signData = RechargeRequestSignTemplate
                   ...
                   .Replace("{type}", bank)             //通道类型11:36
                   .Replace("{amount}", amountStr)      //支付金额
                   .Replace("{notify_url}", notify)     //回调地址
                   .Replace("{success_url}", redirect)  //支付成功返回地址
                   .Replace("{ip}", ip)                 //IP地址
                   .Replace("{key}", key);              //秘钥
            // 加密
            var sign = PaymentHelper.SignByMD5(signData).ToLower();//32位小写


            var data = RechargeRequestDataTemplate
                   .Replace("{mch_id}", shop)           //商户号
                   .Replace("{out_trade_no}", order)    //订单号
                   .Replace("{type}", bank)             //通道类型
                   .Replace("{amount}", amountStr)      //支付金额
                   .Replace("{notify_url}", notify)     //回调地址
                   .Replace("{success_url}", redirect)  //支付成功返回地址
                   .Replace("{ip}", ip)                 //IP地址
            ...
            PaymentHelper.Log($"cbbpay.Recharge\r\nDATA\t{data}\r\nURL\t{url}\r\nPOSTBACK\t{data}");
            #region 封装表单
            ...
            try
            {
                ...
                {
                    string pay_url = obj["pay_url"].ToString();//支付地址
        ...
        public ModelQuery RechargeNotify(string form, string key)
        {
            ...
            try
            {
                ...
                var formData = RechargeNotifySignTemplate
                    ...
                    .Replace("{pay_money}", data.Find(m => m.Key == "pay_money").Value)
                    .Replace("{status}", data.Find(m => m.Key == "status").Value)
                    .Replace("{mch_id}", data.Find(m => m.Key == "mch_id").Value)
                    .Replace("{out_trade_no}", data.Find(m => m.Key == "out_trade_no").Value)
                    .Replace("{order_no}", data.Find(m => m.Key == "order_no").Value)
                    .Replace("{charge_type}", data.Find(m => m.Key == "charge_type").Value)
                    .Replace("{key}", key);
                var sign = PaymentHelper.SignByMD5(formData).ToLower();
                var signFrom = data.Find(m => m.Key == "sign").Value.ToLower();
                PaymentHelper.Log($"cbbpay.CheckSign\tmine:{signFrom}\tfrom:{sign}");
                if (signFrom != sign)
                {
                ...
                result.Amount = Convert.ToDecimal(data.Find(m => m.Key == "pay_money").Value); //单位元
                result.TradeNo = data.Find(m => m.Key == "order_no").Value;//平台订单号(平台)
            ...
            {
                PaymentHelper.Log($"cbbpay.Notify\tmine:{data}");
                PaymentHelper.Log($"cbbpay.Exception\r\n{err.Message}");
