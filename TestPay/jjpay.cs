...
namespace Common.Payment
{
    ...
    /// 注意：请求的价格(单位：分)
    /// </summary>
    public class jjpay : PaymentBase, IPayment
    {
        #region 充值[Recharge]
        /// <summary>
        /// 常量：充值请求待签模板（请求前 签名）
        ...
        protected override string RechargeRequestSignTemplate
        {
            get
            {//支付签名参数
                return "bizType={bizType}&currency={currency}&mchntId={mchntId}&mchntOrderId={mchntOrderId}&notifyUrl={notifyUrl}&orderDesc={orderDesc}&orderSubject={orderSubject}&remarks={remarks}&returnUrl={returnUrl}&sendIp={sendIp}&txnAmt={txnAmt}&txnTime={txnTime}{key}";
            }
        }
        ...
        protected override string RechargeRequestDataTemplate
        {
            get
            {
                return "bizType={bizType}&currency={currency}&mchntId={mchntId}&mchntOrderId={mchntOrderId}&notifyUrl={notifyUrl}&orderDesc={orderDesc}&orderSubject={orderSubject}&remarks={remarks}&returnUrl={returnUrl}&sendIp={sendIp}&signature={signature}&signMethod={signMethod}&txnAmt={txnAmt}&txnTime={txnTime}";
            }
        }
        ...
        protected override string RechargeNotifySignTemplate
        {
            get
            {//参与签名的参数（回调）
                return "code={code}&currency={currency}&mchntId={mchntId}&mchntOrderId={mchntOrderId}&message={message}&orderId={orderId}&remarks={remarks}&success={success}&txnAmt={txnAmt}{key}";
            }
        }
        ...
        {
            var amountStr = (amount * 100).ToString();//金额：分
            var timestr = time.ToString();
            #region 准备数据
            // 参数拼接
            string signData = RechargeRequestSignTemplate
                   .Replace("{mchntId}", shop)           //商户号
                   .Replace("{mchntOrderId}", order)     //商户订单号
                   .Replace("{orderSubject}", order)     //订单标题
                   .Replace("{orderDesc}", order)        //订单描述
                   .Replace("{bizType}", bank)           //业务类型：1微信扫码交易2支付宝扫码交易3 QQ扫码交易4 京东扫码5 银联扫码
                   .Replace("{txnAmt}", amountStr)       //订单金额 分
                   .Replace("{currency}", "CNY")        //币种
                   .Replace("{notifyUrl}", notify)       //回调地址（服务器通知）
                   .Replace("{returnUrl}", redirect)     //页面通知  
                   .Replace("{sendIp}", ip)             //IP客户端
                   .Replace("{txnTime}", timestr)       //时间yyyyMMddHHmmss
                   .Replace("{remarks}", order)         //提交时间
                   .Replace("{key}", key);              //秘钥
            // 加密
            var signature = PaymentHelper.SignByMD5(signData).ToUpper();//32位大写
            ...
            var data = RechargeRequestDataTemplate
                   ...
                   .Replace("{txnTime}", timestr)       //时间yyyyMMddHHmmss
                   .Replace("{remarks}", order)         //提交时间
                   .Replace("{signMethod}", "MD5")      //签名类型
                   .Replace("{signature}", signature);
            #endregion

            PaymentHelper.Log($"jjpay.Recharge\r\nDATA\t{data}\r\nURL\t{url}\r\nPOSTBACK\t{data}");//日志
            #region 封装表单
            var postback = PaymentHelper.Post(url, data);

            Dictionary<string, object> dic = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, object>>(postback);
            if (dic["code"].ToString() == "0000")
            {
             string pay_url = dic["imgUrl"].ToString();//支付地址
             return new RechargeResult() { Status = true, Result = pay_url };
             }
            else
             {
              return new RechargeResult() { Status = false, Result = dic["message"].ToString() };//结果描述
             }
            #endregion
        }
        ...
        public ModelQuery RechargeNotify(string form, string key)
        {
            ...
            {
                #region 验签
                var formData = RechargeNotifySignTemplate//参与签名的参数
                     .Replace("{code}", data.Find(m => m.Key == "code").Value)                          //拓展返回
                     .Replace("{currency}", data.Find(m => m.Key == "currency").Value)                   //币种
                     .Replace("{mchntId}", data.Find(m => m.Key == "mchntId").Value)                       //商户ID
                     .Replace("{mchntOrderId}", data.Find(m => m.Key == "mchntOrderId").Value)             //商户订单号
                     .Replace("{message}", data.Find(m => m.Key == "message").Value)
                     .Replace("{orderId}", data.Find(m => m.Key == "orderId").Value)                      //订单号
                     .Replace("{remarks}", data.Find(m => m.Key == "remarks").Value)
                     .Replace("{success}", data.Find(m => m.Key == "success").Value)                    //成功标识
                     .Replace("{txnAmt}", data.Find(m => m.Key == "txnAmt").Value)                       //订单到账金额
                     .Replace("{key}", key);//签名秘钥

                var jiema = HttpUtility.UrlDecode(formData);//解码
                var resign = PaymentHelper.SignByMD5(jiema).ToUpper();//参数签名
                var signtype= data.Find(m => m.Key == "signMethod").Value;//剔除signMethod
                var signFrom = data.Find(m => m.Key == "signature").Value;//剔除signature
                PaymentHelper.Log($"jjpay.CheckSign\tmine:{signFrom}\tfrom:{resign}");
                if (signFrom != resign)
                {
                ...
                result.OrderNo = data.Find(m => m.Key == "mchntOrderId").Value;//商户订单号(自己).
                result.OrderTime = datetime;
                result.Amount = Convert.ToDecimal(data.Find(m => m.Key == "txnAmt").Value) / 100; //单位分 转元
                result.TradeNo = data.Find(m => m.Key == "orderId").Value;//平台订单号(平台)
            ...
            {
                PaymentHelper.Log($"jjpay.Notify\tmine:{data}");
                PaymentHelper.Log($"jjpay.Exception\r\n{err.Message}");
