...
namespace Common.Payment
{
    ...
    /// 注意：请求的价格(单位：元)
    /// </summary>
    public class kzpay : PaymentBase, IPayment
    {
        #region 充值[Recharge]
        /// <summary>
        /// 常量：充值请求待签模板（请求前 签名）
        ...
        protected override string RechargeRequestSignTemplate
        {
            get
            {
                return "callBackUrl={callBackUrl}&mchNo={mchNo}&mchOrderNo={mchOrderNo}&orderMoney={orderMoney}&payWay={payWay}&key={key}";
            }
        }
        ...
        protected override string RechargeNotifySignTemplate
        {
            get
            {
                return "mchOrderNo={mchOrderNo}&orderMoney={orderMoney}&payTime={payTime}&status={status}";
            }
        }
        ...
        public RechargeResult Recharge(string shop, string bank, string order, DateTime time, decimal amount, string notify, string redirect, string referer, string ip, string key, string info, string url)
        {
            ...
            string signData = RechargeRequestSignTemplate
                .Replace("{callBackUrl}", referer)
                .Replace("{mchNo}", shop)
                .Replace("{mchOrderNo}", order)
                .Replace("{orderMoney}", amountStr)
                .Replace("{payWay}", bank)
                .Replace("{key}", key);
            // 加密
            ...
            PaymentHelper.Log($"kzpay.Recharge\r\nDATA\t{data}\r\nURL\t{url}\r\nPOSTBACK\t{data}");
            #region 封装表单
            try
            {
                var postback = PaymentHelper.Post(url, data, "UTF-8", "application/json");
                if (postback.Contains("/order/error"))
                {
                    return new RechargeResult() { Status = false, Result = postback };
                }
                return new RechargeResult() { Status = true, Result = postback };
            }
            catch (Exception ex)
            {
        ...
        public ModelQuery RechargeNotify(string form, string key)
        {
            ...
            try
            {
                ...
                var formData = RechargeNotifySignTemplate
                    .Replace("{mchOrderNo}", data.Find(m => m.Key == "mchOrderNo").Value)
                    .Replace("{orderMoney}", data.Find(m => m.Key == "orderMoney").Value)
                    .Replace("{payTime}", data.Find(m => m.Key == "payTime").Value)
                    .Replace("{status}", data.Find(m => m.Key == "status").Value)
                    .Replace("{key}", data.Find(m => m.Key == "key").Value);
                var sign = PaymentHelper.SignByMD5(formData).ToUpper();
                var signFrom = data.Find(m => m.Key == "sign").Value.ToUpper();
                PaymentHelper.Log($"kzpay.CheckSign\tmine:{signFrom}\tfrom:{sign}");
                if (signFrom != sign)
                {
                ...
                result.OrderNo = data.Find(m => m.Key == "mchOrderNo").Value;//商户订单号(自己).
                result.OrderTime = datetime;
                result.Amount = Convert.ToDecimal(data.Find(m => m.Key == "orderMoney").Value); //单位元
                result.TradeNo = data.Find(m => m.Key == "mchOrderNo").Value;//平台订单号(平台)
            ...
            {
                PaymentHelper.Log($"kzpay.Notify\tmine:{data}");
                PaymentHelper.Log($"kzpay.Exception\r\n{ex.Message}");
