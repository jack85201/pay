...
namespace Common.Payment
{
    ...
    /// 注意：请求的价格(单位：元)
    /// </summary>
    public class lzpay : PaymentBase, IPayment
    {
        #region 充值[Recharge]
        /// <summary>
        /// 常量：充值请求待签模板（请求前 签名）
        ...
        protected override string RechargeRequestSignTemplate
        {
            get
            {
                return "pay_amount={pay_amount}&pay_applydate={pay_applydate}&pay_bankcode={pay_bankcode}&pay_callbackurl={pay_callbackurl}&pay_memberid={pay_memberid}&pay_notifyurl={pay_notifyurl}&pay_orderid={pay_orderid}&key={key}";
            }
        }
        ...
        protected override string RechargeRequestDataTemplate
        {
            get
            {
                  return "pay_amount={pay_amount}&pay_applydate={pay_applydate}&pay_bankcode={pay_bankcode}&pay_callbackurl={pay_callbackurl}&pay_md5sign={pay_md5sign}&pay_memberid={pay_memberid}&pay_notifyurl={pay_notifyurl}&pay_orderid={pay_orderid}";
            }
        }
        ...
        protected override string RechargeNotifySignTemplate
        {
            get
            {
                return "amount={amount}&attach={attach}&datetime={datetime}&memberid={memberid}&orderid={orderid}&returncode={returncode}&transaction_id={transaction_id}&key={key}";
            }
        }
        ...
        {
            var amountStr = amount.ToString("0.00");//金额：元（保留2位小数）
            var timestr = time.ToString();
            #region 准备数据
            // 参数拼接
            string signData = RechargeRequestSignTemplate
                   .Replace("{pay_memberid}", shop)           //商户号
                   .Replace("{pay_orderid}", order)           //订单号
                   .Replace("{pay_applydate}", timestr)       //提交时间
                   .Replace("{pay_bankcode}", bank)           //银行编码
                   .Replace("{pay_notifyurl}", notify)        //回调地址
                   .Replace("{pay_callbackurl}", redirect)    //支付成功返回地址
                   .Replace("{pay_amount}", amountStr)        //支付金额
                   .Replace("{key}", key);                    //秘钥
            // 加密
            var pay_md5sign = PaymentHelper.SignByMD5(signData).ToUpper();//32位大写
            ...
            var data = RechargeRequestDataTemplate 
                   ...
                   .Replace("{pay_callbackurl}", redirect)     //支付成功返回地址
                   .Replace("{pay_amount}", amountStr)         //支付金额
                   .Replace("{pay_md5sign}", pay_md5sign);     //MD5
            #endregion

            PaymentHelper.Log($"lzpay.Recharge\r\nDATA\t{data}\r\nURL\t{url}\r\nPOSTBACK\t{data}");

            #region 封装表单
            var postback = PaymentHelper.BuildForm(data, url);//From 表单提交
            return new RechargeResult() { Status = true, Result = postback };//返回表单
            #endregion
        }
        ...
        public ModelQuery RechargeNotify(string form, string key)
        {
            ...
            try
            {
                ...
                var formData = RechargeNotifySignTemplate
                    .Replace("{memberid}", data.Find(m => m.Key == "memberid").Value)
                    .Replace("{orderid}", data.Find(m => m.Key == "orderid").Value)
                    .Replace("{amount}", data.Find(m => m.Key == "amount").Value)
                    .Replace("{transaction_id}", data.Find(m => m.Key == "transaction_id").Value)
                    .Replace("{datetime}", data.Find(m => m.Key == "datetime").Value)
                    .Replace("{returncode}", data.Find(m => m.Key == "returncode").Value)
                    .Replace("{attach}", data.Find(m => m.Key == "attach").Value)
                    .Replace("{key}", key);
                var sign = PaymentHelper.SignByMD5(formData).ToUpper();//签名大写
                var signFrom = data.Find(m => m.Key == "sign").Value.ToUpper();//签名大写
                PaymentHelper.Log($"lzpay.CheckSign\tmine:{signFrom}\tfrom:{sign}");
                if (signFrom != sign)
                {
                ...
                result.OrderNo = data.Find(m => m.Key == "transaction_id").Value;//商户订单号(自己).
                result.OrderTime = datetime;
                result.Amount = Convert.ToDecimal(data.Find(m => m.Key == "amount").Value); //单位元
                result.TradeNo = data.Find(m => m.Key == "transaction_id").Value;//平台订单号(平台)
            ...
            {
                PaymentHelper.Log($"lzpay.Notify\tmine:{data}");
                PaymentHelper.Log($"lzpay.Exception\r\n{err.Message}");
