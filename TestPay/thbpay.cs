...
namespace Common.Payment
{
    ...
    /// 注意：请求的价格(单位：分)
    /// </summary>
    public class thbpay : PaymentBase, IPayment
    {
        #region 充值[Recharge]
        /// <summary>
        /// 常量：充值请求待签模板（请求前 签名）
        ...
        protected override string RechargeRequestSignTemplate
        {
            get
            {
                return "applydate={applydate}&notify_url={notify_url}&out_trade_no={out_trade_no}&pay_amount={pay_amount}&product_id={product_id}&remark={remark}&return_url={return_url}&subject={subject}&user_id={user_id}&apikey={apikey}";
            }
        }
        ...
        protected override string RechargeRequestDataTemplate
        {
            get
            {
                return "applydate={applydate}&notify_url={notify_url}&out_trade_no={out_trade_no}&pay_amount={pay_amount}&product_id={product_id}&remark={remark}&return_url={return_url}&sign={sign}&subject={subject}&user_id={user_id}";
            }
        }
        ...
        protected override string RechargeNotifySignTemplate
        {
            get
            {
                return "datetime={datetime}&out_trade_no={out_trade_no}&returncode={returncode}&trade_no={trade_no}&transaction_money={transaction_money}&user_id={user_id}&apikey={apikey}";
            }
        }
        ...
        public RechargeResult Recharge(string shop, string bank, string order, DateTime time, decimal amount, string notify, string redirect, string referer, string ip, string key, string info, string url)
        {
            ...
            string signData = RechargeRequestSignTemplate
                   .Replace("{user_id}", shop)           //商户号
                   .Replace("{out_trade_no}", order)     //商户订单号
                   .Replace("{product_id}", bank)       //支付产品
                   .Replace("{return_url}", redirect)   //页面通知
                   .Replace("{notify_url}", notify)     //回调地址（服务器通知）
                   .Replace("{subject}", order)         //订单标题
                   .Replace("{remark}", order)          //备注
                   .Replace("{pay_amount}", amountStr)  //订单金额
                   .Replace("{applydate}", timestr)     //提交时间
                  .Replace("{apikey}", key);            //秘钥
            // 加密
            var sign = PaymentHelper.SignByMD5(signData).ToUpper();//32位小写

            var data = RechargeRequestDataTemplate
                   .Replace("{user_id}", shop)          //商户号
                   .Replace("{out_trade_no}", order)    //商户订单号
                   .Replace("{product_id}", bank)       //支付产品
                   .Replace("{return_url}", redirect)   //支付成功返回地址
                   .Replace("{notify_url}", notify)     //回调地址（服务器通知）
                   .Replace("{subject}", order)         //订单标题
                   .Replace("{remark}", order)          //备注
                   .Replace("{pay_amount}", amountStr)  //订单金额
            ...
            PaymentHelper.Log($"thbpay.Recharge\r\nDATA\t{data}\r\nURL\t{url}\r\nPOSTBACK\t{data}");
            #region 封装表单
            ...
            try
            {
                ...
                {
                    string datas = obj["data"].ToString();//提取JSON层的{}
                    Dictionary<string, object> dic = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, object>>(datas);
                    string datax = dic["pay_extends"].ToString();
                    Dictionary<string, object> rdic = Newtonsoft.Json.JsonConvert.DeserializeObject<Dictionary<string, object>>(datax);
                    string pay_url = rdic["pay_url"].ToString();//支付地址
        ...
        public ModelQuery RechargeNotify(string form, string key)
        {
            ...
            try
            {
                ...
                var formData = RechargeNotifySignTemplate
                    .Replace("{user_id}", data.Find(m => m.Key == "user_id").Value)                       //商户ID
                    .Replace("{trade_no}", data.Find(m => m.Key == "trade_no").Value)                     //订单号
                    .Replace("{out_trade_no}", data.Find(m => m.Key == "out_trade_no").Value)             //商户订单号
                    .Replace("{transaction_money}", data.Find(m => m.Key == "transaction_money").Value)   //订单到账金额
                    .Replace("{datetime}", data.Find(m => m.Key == "datetime").Value)                     //交易时间
                    .Replace("{returncode}", data.Find(m => m.Key == "returncode").Value)                 //交易状态'00'为成功
                    .Replace("{apikey}", key);

                var jiema = HttpUtility.UrlDecode(formData);//解码
                var sign = PaymentHelper.SignByMD5(jiema).ToUpper();                                  //签名大写
                var signFrom = data.Find(m => m.Key == "sign").Value.ToUpper();
                PaymentHelper.Log($"thbpay.CheckSign\tmine:{signFrom}\tfrom:{sign}");
                if (signFrom != sign)
                {
                ...
                result.OrderNo = data.Find(m => m.Key == "out_trade_no").Value;//商户订单号(自己).
                result.OrderTime = datetime;
                result.Amount = Convert.ToDecimal(data.Find(m => m.Key == "transaction_money").Value) / 100; //单位分 转元
                result.TradeNo = data.Find(m => m.Key == "trade_no").Value;//平台订单号(平台)
            ...
            {
                PaymentHelper.Log($"thbpay.Notify\tmine:{data}");
                PaymentHelper.Log($"thbpay.Exception\r\n{err.Message}");
